...
serve(async (req) => {
  ...
  try {
    ...
    const expectedInternalSecret = Deno.env.get("CAMPAIGN_DISPATCHER_SECRET") ?? "";
    const receivedInternalSecret = req.headers.get("x-internal-secret") ?? "";
    const isInternalCall = !!expectedInternalSecret && receivedInternalSecret === expectedInternalSecret;
    const isServiceRole = bearerToken === serviceRoleKey || isInternalCall;
    ...
    if (!publicationId) return jsonResponse({ error: "publicationId is required" }, 400);

    const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey);
    let publicationQuery = supabaseAdmin.from("publications").select("*").eq("id", publicationId);
    if (!isServiceRole) publicationQuery = publicationQuery.eq("user_id", userId!);
    const { data: pub, error: pubError } = await publicationQuery.maybeSingle();
    if (pubError) return jsonResponse({ error: "Publication lookup failed", detail: pubError.message }, 500);
    ...
    if (accountError) return jsonResponse({ error: "Social account lookup failed", detail: accountError.message }, 500);
    if (!account) return jsonResponse({ error: "Social account not found for publication" }, 400);
    if (account.provider !== "postpeer") return jsonResponse({ error: "Social account is not connected through PostPeer" }, 400);
    if (!account.provider_connection_id) return jsonResponse({ error: "PostPeer integration ID is missing" }, 400);

    let content: any = null;
    if (pub.content_id) {
      const { data, error } = await supabaseAdmin.from("content_library").select("id,storage_path").eq("id", pub.content_id).maybeSingle();
      if (error) return jsonResponse({ error: "Content lookup failed", detail: error.message }, 500);
      content = data;
    }

    let render: any = null;
    if (pub.media_render_id) {
      const { data, error } = await supabaseAdmin.from("media_renders").select("id,storage_path,status").eq("id", pub.media_render_id).maybeSingle();
      if (error) return jsonResponse({ error: "Render lookup failed", detail: error.message }, 500);
      render = data;
    }

    const platform = pub.platform || account.platform;
    if (!platform) return jsonResponse({ error: "Publication platform is missing" }, 400);
    ...
    const bucketName = renderPath ? "rendered" : "content-library";
    ...
    const platformConfig = normalizedPlatform === "youtube"
      ...
      : { platform, accountId: account.provider_connection_id };
    ...
    const rawProviderStatus = typeof response.status === "string" ? response.status.toLowerCase() : "processing";
    const providerStatus = rawProviderStatus === "failed" || rawProviderStatus === "cancelled" ? rawProviderStatus : response.publishedAt ? "published" : "processing";
    const updatePayload: Record<string, unknown> = { provider_post_id: response.postId, status: providerStatus, updated_at: new Date().toISOString() };
    const platformResult = response.platforms?.[0];
    if (platformResult?.platformPostUrl) updatePayload.post_url = platformResult.platformPostUrl;
    if (platformResult?.error) {
      updatePayload.status = "failed";
      updatePayload.last_error = typeof platformResult.error === "string" ? platformResult.error : JSON.stringify(platformResult.error);
    }
    if (response.publishedAt) updatePayload.published_at = response.publishedAt;

    const { error: updateError } = await supabaseAdmin.from("publications").update(updatePayload).eq("id", publicationId);
    if (updateError) {
      console.error("[postpeer-post-create] Provider created but DB update failed:", updateError.message, "provider_post_id:", response.postId);
