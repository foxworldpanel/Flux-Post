ALTER TABLE public.ai_studio_scripts
  ADD COLUMN IF NOT EXISTS subtitle_font text NOT NULL DEFAULT 'DejaVu Sans',
  ADD COLUMN IF NOT EXISTS subtitle_font_size integer NOT NULL DEFAULT 22,
  ADD COLUMN IF NOT EXISTS subtitle_color text NOT NULL DEFAULT 'white',
  ADD COLUMN IF NOT EXISTS subtitle_position text NOT NULL DEFAULT 'bottom';

ALTER TABLE public.ai_studio_scripts
  DROP CONSTRAINT IF EXISTS ai_studio_scripts_subtitle_font_size_check,
  DROP CONSTRAINT IF EXISTS ai_studio_scripts_subtitle_color_check,
  DROP CONSTRAINT IF EXISTS ai_studio_scripts_subtitle_position_check;

ALTER TABLE public.ai_studio_scripts
  ADD CONSTRAINT ai_studio_scripts_subtitle_font_size_check
    CHECK (subtitle_font_size BETWEEN 16 AND 36),
  ADD CONSTRAINT ai_studio_scripts_subtitle_color_check
    CHECK (subtitle_color IN ('white', 'yellow', 'cyan', 'pink')),
  ADD CONSTRAINT ai_studio_scripts_subtitle_position_check
    CHECK (subtitle_position IN ('top', 'center', 'bottom'));
...
BEGIN
  ...
  v_render_key := concat_ws(
    ':', 'ai-studio-v2', p_script_id, v_voice.id, p_content_id,
    p_music_track_id, v_music_volume, p_subtitles_enabled,
  ...
  IF NOT FOUND THEN
    INSERT INTO public.media_renders (
      ...
      v_user_id, v_render_key, p_content_id, p_music_track_id, 0,
      'narration_plus_music', v_music_volume, 0, 'ai_studio_vertical_v2',
      jsonb_build_object(
        'pipeline', 'ai_studio_v2',
...
REVOKE ALL ON FUNCTION public.start_ai_studio_render(
  uuid, uuid, uuid, integer, boolean, text, integer, text, text
) FROM PUBLIC;
