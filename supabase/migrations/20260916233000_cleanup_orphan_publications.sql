-- Final deletion semantics: finished posts stay in permanent history; every
-- non-finished stale job is removable even when a provider id was allocated.
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Campanha não encontrada ou sem permissão'; END IF;
  ...
  WHERE campaign_id = p_campaign_id AND content_id IS NOT NULL;
  ...
  SELECT count(*) INTO v_preserved_publications
  FROM public.publications
  WHERE campaign_id = p_campaign_id;
  ...
  WHERE content.user_id = v_user
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.publications AS publication
  ...
  WHERE track.user_id = v_user
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.campanhas AS campaign
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'removed_publications', v_removed
  );
END;
$$;
