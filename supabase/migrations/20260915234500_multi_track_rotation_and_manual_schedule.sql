-- Campaign V3: multiple music tracks with deterministic rotation and real manual schedules.
-- Keeps the legacy music_track_id column as the primary/first track for compatibility.

ALTER TABLE public.campanhas
  ADD COLUMN IF NOT EXISTS music_track_ids uuid[] NOT NULL DEFAULT ARRAY[]::uuid[],
  ADD COLUMN IF NOT EXISTS manual_daily_times time[] NOT NULL DEFAULT ARRAY[]::time[];

UPDATE public.campanhas
SET music_track_ids = ARRAY[music_track_id]
WHERE music_track_id IS NOT NULL
  AND cardinality(music_track_ids) = 0;

CREATE INDEX IF NOT EXISTS idx_campanhas_music_track_ids
  ON public.campanhas USING gin (music_track_ids);
...
BEGIN
  ...
  IF cardinality(v_music_ids) < (p_campaign->>'posts_por_dia')::int THEN
  ...
  RETURN jsonb_build_object('ok',true,'campaign_id',v_campaign_id,'publication_count',jsonb_array_length(p_publications),'music_track_count',cardinality(v_music_ids));
END;
$$;
...
BEGIN
  ...
  IF v_pub.content_id IS NOT NULL THEN
    SELECT
      NOT EXISTS (SELECT 1 FROM public.publications p WHERE p.content_id=v_pub.content_id AND p.user_id=v_pub.user_id AND COALESCE(p.status,'') NOT IN ('published','failed','cancelled','canceled')),
      EXISTS (SELECT 1 FROM public.publications p WHERE p.content_id=v_pub.content_id AND p.user_id=v_pub.user_id AND (p.status='published' OR p.provider_post_id IS NOT NULL))
  ...
  IF v_pub.campaign_id IS NOT NULL THEN
    ...
    IF v_all_terminal THEN
      SELECT CASE WHEN cardinality(c.music_track_ids)>0 THEN c.music_track_ids WHEN c.music_track_id IS NOT NULL THEN ARRAY[c.music_track_id] ELSE ARRAY[]::uuid[] END
      INTO v_music_ids FROM public.campanhas c WHERE c.id=v_pub.campaign_id;
      ...
      FOREACH v_music_id IN ARRAY v_music_ids
      LOOP
        IF NOT EXISTS (
          SELECT 1 FROM public.campanhas c
          WHERE c.id<>v_pub.campaign_id AND c.status IN ('ativo','pausado')
            AND (c.music_track_id=v_music_id OR c.music_track_ids @> ARRAY[v_music_id])
        ) THEN
          UPDATE public.music_tracks SET campanha_ativa=false
          WHERE id=v_music_id AND user_id=v_pub.user_id;
        END IF;
      END LOOP;
    END IF;
  END IF;
