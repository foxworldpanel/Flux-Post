-- Permanent scheduling safety net: the same content can never be scheduled twice
-- for the same social account. Existing historical duplicates are preserved;
-- the trigger applies to new/changed rows only.
-- The final provider claim lives in the later publication_safety_claim migration.
