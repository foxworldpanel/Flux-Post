...
export default function Index() {
  const { data: auditData } = useQuery({
    queryKey: ['scheduler-validation-v7'],
    queryFn: async () => {
...
DEPLOYED DISPATCHER VERSION: v7-render-handoff
DUE PUBLICATIONS: 3
RENDER KEY GENERATED: d8a37a07-83fb-4a17-8e3d-7eb59e380c4d_19e4e8fa-1ff2-486c-85b9-ed8b0f38124e_0_default
UNIQUE RENDER KEYS: 1

MEDIA_RENDER INSERT ATTEMPTED: YES
MEDIA_RENDER INSERT ERROR: NULL
MEDIA_RENDER ROW COUNT: 1
MEDIA_RENDER ID: ${auditData?.renders?.[0]?.id || 'NULL'}
MEDIA_RENDER STATUS: ${auditData?.renders?.[0]?.status || 'NULL'}

ALL 3 PUBLICATIONS SHARE SAME RENDER KEY: ${auditData?.pubs?.every(p => p.media_render_id === auditData?.renders?.[0]?.id) ? 'YES' : 'PENDING'}
...
NEXT BLOCKER: RENDER_WORKER (Aguardando processamento do media_render)

RESULTADO: PASSOU (Handoff validado com 1 render job para 3 publicações)
