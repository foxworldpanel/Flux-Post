...
import { Search, Play, Check, Loader2, ExternalLink, Library, ChevronLeft, ChevronRight, Filter } from 'lucide-react'
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400">Descubra e importe conteúdos virais do Pexels para sua biblioteca</p>
          </div>
        ...
        {results.length > 0 ? (
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {results.map((video) => (
                <Card key={video.id} className="bg-[#13131F] border-white/5 overflow-hidden group hover:border-purple-500/30 transition-all duration-300">
                  ...
                  <CardContent className="p-4 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <p className="text-white font-medium text-sm truncate max-w-[150px]">{video.user.name}</p>
                        <p className="text-slate-500 text-[10px] uppercase tracking-wider">Fonte: Pexels</p>
                      </div>
                      <p className="text-slate-400 text-xs font-mono">{video.width}x{video.height}</p>
                    </div>
                    <div className="flex gap-2">
                      ...
                      >
                        {importingId === video.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 
                         importedIds.has(video.id) ? <Check className="w-4 h-4 mr-1" /> : 'Aprovar'}
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-8 pb-12">
              <p className="text-slate-500 text-sm">
                Mostrando <span className="text-white font-medium">{(page - 1) * 20 + 1} - {Math.min(page * 20, totalResults)}</span> de <span className="text-white font-medium">{totalResults}</span> resultados
              </p>
          ...
          <div className="min-h-[400px] flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-3xl bg-[#13131F]/30 text-center px-6">
            ...
            <p className="text-slate-500 max-w-sm mb-8">Digite termos relacionados ao seu nicho para encontrar os melhores vídeos para suas campanhas.</p>
            <div className="flex flex-wrap justify-center gap-2 max-w-md">
              {['chocolate cake', 'cooking', 'nature', 'fitness', 'cars', 'lifestyle'].map(term => (
                <Badge 
                  key={term} 
                  variant="outline" 
        ...
        <Card className="bg-gradient-to-br from-[#13131F] to-[#0A0A0F] border-white/5 overflow-hidden group">
          <CardHeader className="border-b border-white/5 bg-white/5">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-space font-bold text-white">Garimpo Automático</CardTitle>
                <CardDescription className="text-slate-400">Em breve: Deixe nossa IA minerar conteúdos para você 24/7</CardDescription>
              </div>
              <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20">Alpha Access</Badge>
            </div>
          </CardHeader>
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="space-y-2">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Estoque Mínimo</p>
                <p className="text-2xl font-space font-bold text-white">10</p>
              </div>
              <div className="space-y-2">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Fontes Ativas</p>
                <div className="flex gap-1">
                  <Badge variant="secondary" className="bg-white/5 border-white/10 text-xs">Pexels</Badge>
                </div>
              </div>
              <div className="flex items-center">
                <Button variant="outline" className="w-full border-dashed border-white/10 hover:border-purple-500/50 hover:bg-purple-500/5 transition-all h-12">
          ...
          <DialogContent className="max-w-4xl bg-[#0A0A0F] border-white/10 p-0 overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-3">
              <div className="md:col-span-2 bg-black flex items-center justify-center min-h-[500px]">
                {selectedVideo && (
                  <video 
                    src={selectedVideo.video_files.find(f => f.width < f.height)?.link || selectedVideo.video_files[0]?.link} 
                    controls 
                    autoPlay 
              ...
              <div className="p-8 space-y-8 bg-[#13131F]/50 backdrop-blur-xl border-l border-white/5">
                <div className="space-y-4">
                  <DialogTitle className="text-2xl font-space font-bold text-white">Detalhes do Vídeo</DialogTitle>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400">Criador:</span>
                      <a href={selectedVideo?.user.url} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 flex items-center gap-1 transition-colors">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-white font-medium">{selectedVideo?.width} x {selectedVideo?.height}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Categoria para Importação</label>
                    <Select value={category} onValueChange={setCategory}>
                ...
                <div className="pt-8">
                  <Button 
                    ...
                    disabled={importingId === selectedVideo?.id || importedIds.has(selectedVideo?.id || 0)}
                    onClick={() => selectedVideo && handleImport(selectedVideo)}
                  >
                    {importingId === selectedVideo?.id ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : 
                     importedIds.has(selectedVideo?.id || 0) ? <Check className="w-5 h-5 mr-2" /> : <Check className="w-5 h-5 mr-2" />}
                    {importedIds.has(selectedVideo?.id || 0) ? 'Já na Biblioteca' : 'Aprovar e Importar'}
                  </Button>
                </div>
              </div>
            </div>
